...
- `ri-llm-provider`
  ...
  - Built-in model registry seed and thinking-level helpers, including
    ...
    generated-catalog metadata parity for the current M2.7 model slice, Zai
...
Current Rust tests: 1194 enumerated by `cargo test --workspace -- --list`.

- `ri-llm-provider`: 987 tests: 2 library tests, 354 `provider_core` tests, and
  631 `provider_live` tests. This is 266 above the 721 direct simple source
...
- The raw 1194-vs-871 count is not completion proof. Rust tests sometimes
...
- Test parity is not certified by raw count alone: 1194 Rust tests cover the
