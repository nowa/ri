...
- `ri-llm-provider`
  ...
  - Built-in model registry seed and thinking-level helpers, including
    ...
    the current V4 model slice, Cerebras generated-catalog metadata parity for
  ...
  - OpenAI Codex OAuth helpers for authorization URL construction,
    ...
    token exchange primitives, source-style 16-byte lowercase-hex state
    generation and Codex token expiry without the Anthropic/GitHub 5-minute
    early-refresh offset, ChatGPT JWT account-id validation, and source-style
    `accountId` credential preservation.
...
Current Rust tests: 1208 enumerated by `cargo test --workspace -- --list`.

- `ri-llm-provider`: 1001 tests: 2 library tests, 362 `provider_core` tests, and
  637 `provider_live` tests. This is 280 above the 721 direct simple source
...
- The raw 1208-vs-871 count is not completion proof. Rust tests sometimes
...
- Latest local verification on 2026-05-22 after aligning Pi
  ...
  with `gpt-4o`) and Google (`gemini-2.0-flash`), require Anthropic API-key and
  OAuth repeated-prompt cache activity on `claude-sonnet-4-5` and
  `claude-sonnet-4-6`, and add the GitHub Copilot Anthropic total-usage path on
  `claude-sonnet-4.6` without source/Markdown scanner tests:
  `cargo fmt --check`,
  `cargo test -p ri-llm-provider --test provider_live reports_total_usage_components -- --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_live -- --list`, and
...
- Latest local verification on 2026-05-22 after aligning `providers/mistral.ts`
  ...
  `HTTP status` fallback for empty response bodies, and truncate raw error
  bodies at 4000 characters with the omitted-character count:
  `cargo test -p ri-llm-provider --test provider_core builtin_mistral_provider_formats_http_errors_with_status_and_truncated_body -- --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core -- --test-threads=1`,
  `cargo test -p ri-llm-provider -- --test-threads=1`,
  `cargo test --workspace -- --list`, `cargo fmt --check`, and
  `git diff --check` passed; the workspace list enumerated 1176 tests.
...
- Latest local verification on 2026-05-22 after aligning Pi
  ...
  `cargo fmt --check`, and `git diff --check` passed.
- Latest local verification on 2026-05-22 after aligning Pi
  `utils/oauth/openai-codex.ts` state generation: CLI Codex OAuth now uses 16
  random bytes encoded as 32 lowercase hex characters, matching
  `randomBytes(16).toString("hex")` instead of reusing a PKCE verifier prefix:
  `cargo fmt`,
  `cargo test -p ri-llm-provider --test provider_core openai_codex_oauth -- --test-threads=1`,
...
- Latest local verification on 2026-05-22 after aligning Pi
  ...
  tests.
- Latest local verification on 2026-05-22 after aligning Pi
  `models.generated.ts` OpenAI and Azure OpenAI Responses o-series catalog
  metadata: Rust now exposes the generated o1/o3/o4 model slice for both
  providers, maps the source API/base URL shape, reasoning flag, empty
  thinking-level map, text-only `o3-mini` input capability, context/output
  windows, and non-zero usage cost table:
  `cargo fmt`,
  `cargo test -p ri-llm-provider --test provider_core openai_and_azure_o_series_model_metadata_matches_generated_catalog -- --exact --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core openai_model_metadata_matches_generated_gpt5_catalog -- --exact --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core azure_openai_model_metadata_matches_generated_gpt5_catalog -- --exact --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core openai_responses_ -- --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core azure_openai -- --test-threads=1`,
...
- Latest local verification on 2026-05-22 after aligning Pi
  ...
  tests.
- Latest local verification on 2026-05-22 after aligning Pi
  `models.generated.ts` Cerebras, Minimax, and Minimax-CN catalog metadata:
  Rust now exposes the generated Cerebras `llama3.1-8b` and `zai-glm-4.7`
  entries, the generated Minimax highspeed entry, both generated Minimax-CN
  entries, and maps the source API/base URL, text-only input, reasoning flags,
  context/output windows, and usage cost tables for the current Cerebras and
  M2.7 model slices:
  `cargo fmt`,
  `cargo test -p ri-llm-provider --test provider_core cerebras_model_metadata_matches_generated_catalog -- --exact --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core minimax_model_metadata_matches_generated_catalog -- --exact --test-threads=1`,
  ...
  `cargo test -p ri-llm-provider --test provider_core -- --test-threads=1`,
  `cargo fmt --check`, `git diff --check`, and
  `cargo test --workspace -- --list` passed; the list command enumerated 1194
  tests.
- Latest local verification on 2026-05-22 after aligning Pi
  `models.generated.ts` xAI catalog metadata: Rust now exposes the generated
  Grok 2/3/4/code/vision model entries and maps the source API/base URL,
  text/image input support, reasoning flags, context/output windows, and usage
  cost tables for the current xAI model slice:
  `cargo fmt`,
  `cargo test -p ri-llm-provider --test provider_core xai_model_metadata_matches_generated_catalog -- --exact --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core -- --test-threads=1`,
  `cargo fmt --check`, `git diff --check`, and
...
- Latest local verification on 2026-05-22 after aligning Pi
  ...
  tests.
- Latest local verification on 2026-05-22 after aligning Pi
  `models.generated.ts` Google Vertex catalog metadata: Rust now exposes the
  generated Gemini 1.5/2.0/2.5/3.x Vertex model slice and maps the source
  Google Vertex API/base URL, text/image input support, reasoning flags,
  thinking-level maps, context/output windows, and usage cost tables:
  `cargo fmt`,
  `cargo test -p ri-llm-provider --test provider_core google_vertex_model_metadata_matches_generated_catalog -- --exact --test-threads=1`,
  ...
  `cargo test -p ri-llm-provider --test provider_core google_vertex_client_config_resolves_api_keys_adc_and_custom_base_urls -- --exact --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core -- --test-threads=1`,
  `cargo fmt --check`, `git diff --check`, and
  `cargo test --workspace -- --list` passed; the list command enumerated 1198
  tests.
- Latest local verification on 2026-05-22 after aligning Pi
  `models.generated.ts` Google/Gemini catalog metadata: Rust now exposes the
  generated Gemini 1.5/2.0/2.5/3.x, live Gemini, and Gemma model slice and maps
  the source Google Generative AI API/base URL, text/image input support,
  reasoning flags, thinking-level maps, context/output windows, and usage cost
  tables:
  `cargo fmt`,
  `cargo test -p ri-llm-provider --test provider_core google_model_metadata_matches_generated_catalog -- --exact --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core google_simple_payload_maps_reasoning_to_budget_or_level -- --exact --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core google_simple_payload_disables_thinking_for_gemini_reasoning_models -- --exact --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core google_vertex_model_metadata_matches_generated_catalog -- --exact --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core registered_model_apis_have_builtin_provider_implementations -- --exact --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core -- --test-threads=1`,
  `cargo fmt --check`, `git diff --check`, and
...
- Latest local verification on 2026-05-22 after aligning Pi
  ...
  tests.
- Latest local verification on 2026-05-22 after aligning Pi
  `models.generated.ts` Groq catalog metadata: Rust now exposes the generated
  DeepSeek, Gemma, Compound, Llama, Mistral, Kimi, GPT OSS, and Qwen model
  slice and maps the source OpenAI-compatible API/base URL, text/image input
  support, reasoning flags, Qwen thinking-level map, context/output windows,
  and usage cost tables:
  `cargo fmt`,
  `cargo test -p ri-llm-provider --test provider_core groq_model_metadata_matches_generated_catalog -- --exact --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core openai_completions_payload_keeps_normal_groq_reasoning_effort_without_mapping -- --exact --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core openai_completions_payload_maps_reasoning_and_zai_tool_stream_compat -- --exact --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core registered_model_apis_have_builtin_provider_implementations -- --exact --test-threads=1`,
  `cargo test -p ri-llm-provider --test provider_core -- --test-threads=1`,
  `cargo fmt --check`, `git diff --check`, and
