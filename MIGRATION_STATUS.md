...
- `ri-llm-provider`
  ...
  - Built-in model registry seed and thinking-level helpers, including
    ...
    parity for the current Claude/Gemini/GPT/Grok model slice, Google Vertex
...
Current Rust tests: 1198 enumerated by `cargo test --workspace -- --list`.

- `ri-llm-provider`: 991 tests: 2 library tests, 358 `provider_core` tests, and
  631 `provider_live` tests. This is 270 above the 721 direct simple source
...
- The raw 1198-vs-871 count is not completion proof. Rust tests sometimes
...
- Test parity is not certified by raw count alone: 1198 Rust tests cover the
